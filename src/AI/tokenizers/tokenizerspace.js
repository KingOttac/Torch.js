function tokenizerspace(lines) {
	...
	tokens[0] = "\n";
	let convertedlines = [];
	for (let a = 0; convertedlines.length < learningset+sampleset && a < lines.length; a++) {
		
		let listarr = [];
		listarr = lines[a];
		listarr = split(listarr," ");
		for (let b = 0; b < listarr.length-1; b++) {
			listarr[b] += " ";
		}
		if (listarr[0] != "") {
		...
		for (let b = 0; b < listarr.length && convertedlines.length < learningset+sampleset; b++) {
	...
	for (let a = 0; a < convertedlines.length; a++) {
